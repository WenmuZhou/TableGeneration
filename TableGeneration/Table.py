...
class Table:
    def __init__(self,
                 ...
                 cell_max_width=0,
                 cell_max_height=0):
        assert cell_box_type in ['cell', 'text'], "cell_box_type must in ['cell', 'text'],cell: use cell location as cell box; text: use location of text in cell as cell box"
        ...
        self.pre_boder_style = {
            1: {'name': 'border', 'style': 'border:1px solid black;'},  # 绘制全部边框
            2: {'name': 'border_top', 'style': 'border-top:1px solid black;'},  # 绘制上横线
            3: {'name': 'border_bottom', 'style': 'border-bottom:1px solid black;'},  # 绘制下横线
            4: {'name': 'head_border_bottom', 'style': '}th{border-bottom: 1px solid black;'},  # 绘制 head 下横线
            5: {'name': 'no_border', 'style': ''},  # 无边框
            6: {'name': 'border_left', 'style': 'border-left:1px solid black;'},  # 绘制左竖线
            7: {'name': 'border_right', 'style': 'border-right:1px solid black;'}  # 绘制右竖线
        }
    ...
    def define_col_types(self):
        ...
        for i, type in enumerate(
                random.choices(['n', 'm', 'c', 'e'],
                               weights=[prob_numbers, prob_money, prob_words, prob_ens], k=self.no_of_cols)):
    ...
    def generate_random_text(self, type):
        ...
        if type in ['n', 'm']:
            ...
            if random.random() < 0.5:
                ...
                # 随机保留小数点后2位
                out = str(random.random() * max_num)[:len(str(max_num)) + random.randint(0, 3)]
            if type == 'm':
                out = '$' + out
        elif (type == 'e'):
    ...
    def make_first_col_spans(self):
        ...
        span_indices, span_lengths = self.agnostic_span_indices(self.no_of_rows - 2, self.max_span_row_count)
    ...
    def create_style(self):
        ...
        # 表格中文本左右对齐方式
        style += "text-align:{};".format(random.choices(['left', 'right', 'center'], weights=[0.25, 0.25, 0.5])[0])
        ...
        if self.cell_max_height != 0:
            style += "height: {}px;".format(random.randint(self.cell_max_height // 2, self.cell_max_height))
        if self.cell_max_width != 0:
            style += "width: {}px;".format(random.randint(self.cell_max_width // 2, self.cell_max_width))
        # 文本换行
        ...
        # 绘制边框
        style += self.pre_boder_style[self.border_type]['style']
        style += '}'
        ...
        for r in range(self.no_of_rows):
            ...
            for c in range(self.no_of_cols):
                ...
                if self.cell_box_type == 'text':
                    txt = '<span id=' + str(self.idcounter) + '>' + txt + ' </span>'
                self.idcounter += 1
                html += txt + '</' + htmlcol + '>'
                structure.append('</td>')
    ...
    def create(self):
        ...
        html, structure = self.create_html()  # create equivalent html

        return self.idcounter, html, structure, self.pre_boder_style[self.border_type]['name']
